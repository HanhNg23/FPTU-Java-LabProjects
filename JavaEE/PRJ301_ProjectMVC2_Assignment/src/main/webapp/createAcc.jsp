	...
	<div>
		<form action="DispatchServlet" method="POST">
			<c:if test="${not empty err.usernameLengthOrExistsErr}">
				<font color="red"> ${err.usernameLengthOrExistsErr} </font>
				<br>
			</c:if>
			...
			<c:if test="${not empty err.passwordLengthErr}">
				<font color="red"> ${err.passwordLengthErr} </font>
				<br>
			</c:if>
			Password * <input type="password" name="txtPassword"
				value="${param.txtPassword}" />(5-20 characters)<br>
			<br>
			...
			<c:if test="${not empty err.fullnameLengthErr}">
				<font color="red"> ${err.fullnameLengthErr} </font>
				<br>
			</c:if>
			...
			<br> <input type="reset" value="Reset" /> <input type="submit"
				name="btAction" value="Create Account" />
		</form>
